 ...
 * - [AOutbDeliveryPartnerCreateFragment]: to create or edit a(n) AOutbDeliveryPartnerType,
 * - [AOutbDeliveryPartnerDetailFragment]: to display the details of a(n) AOutbDeliveryPartnerType,
 * - [AOutbDeliveryPartnerListFragment]: to list AOutbDeliveryPartner.
 *
 ...
 * what to do.
 */
class AOutbDeliveryPartnerActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<AOutbDeliveryPartnerType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryPartnerListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: AOutbDeliveryPartnerType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link AOutbDeliveryPartnerCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: AOutbDeliveryPartnerType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryPartnerDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, AOutbDeliveryPartnerDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = AOutbDeliveryPartnerCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: AOutbDeliveryPartnerType?) {
        val updateFragment = AOutbDeliveryPartnerCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as AOutbDeliveryPartnerActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[AOutbDeliveryPartnerTypeViewModel::class.java]
