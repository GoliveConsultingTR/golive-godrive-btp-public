 ...
 * - [AOutbDeliveryAddressCreateFragment]: to create or edit a(n) AOutbDeliveryAddressType,
 * - [AOutbDeliveryAddressDetailFragment]: to display the details of a(n) AOutbDeliveryAddressType,
 * - [AOutbDeliveryAddressListFragment]: to list AOutbDeliveryAddress.
 *
 ...
 * what to do.
 */
class AOutbDeliveryAddressActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<AOutbDeliveryAddressType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryAddressListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: AOutbDeliveryAddressType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link AOutbDeliveryAddressCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: AOutbDeliveryAddressType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryAddressDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, AOutbDeliveryAddressDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = AOutbDeliveryAddressCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: AOutbDeliveryAddressType?) {
        val updateFragment = AOutbDeliveryAddressCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as AOutbDeliveryAddressActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[AOutbDeliveryAddressTypeViewModel::class.java]
