 ...
 * - [AOutbDeliveryItemCreateFragment]: to create or edit a(n) AOutbDeliveryItemType,
 * - [AOutbDeliveryItemDetailFragment]: to display the details of a(n) AOutbDeliveryItemType,
 * - [AOutbDeliveryItemListFragment]: to list AOutbDeliveryItem.
 *
 ...
 * what to do.
 */
class AOutbDeliveryItemActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<AOutbDeliveryItemType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryItemListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: AOutbDeliveryItemType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link AOutbDeliveryItemCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: AOutbDeliveryItemType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryItemDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, AOutbDeliveryItemDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = AOutbDeliveryItemCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: AOutbDeliveryItemType?) {
        val updateFragment = AOutbDeliveryItemCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as AOutbDeliveryItemActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[AOutbDeliveryItemTypeViewModel::class.java]
