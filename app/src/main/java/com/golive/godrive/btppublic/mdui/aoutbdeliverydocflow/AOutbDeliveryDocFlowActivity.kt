 ...
 * - [AOutbDeliveryDocFlowCreateFragment]: to create or edit a(n) AOutbDeliveryDocFlowType,
 * - [AOutbDeliveryDocFlowDetailFragment]: to display the details of a(n) AOutbDeliveryDocFlowType,
 * - [AOutbDeliveryDocFlowListFragment]: to list AOutbDeliveryDocFlow.
 *
 ...
 * what to do.
 */
class AOutbDeliveryDocFlowActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<AOutbDeliveryDocFlowType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryDocFlowListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: AOutbDeliveryDocFlowType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link AOutbDeliveryDocFlowCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: AOutbDeliveryDocFlowType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryDocFlowDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, AOutbDeliveryDocFlowDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = AOutbDeliveryDocFlowCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: AOutbDeliveryDocFlowType?) {
        val updateFragment = AOutbDeliveryDocFlowCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as AOutbDeliveryDocFlowActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[AOutbDeliveryDocFlowTypeViewModel::class.java]
