 ...
 * - [AHandlingUnitItemDeliveryCreateFragment]: to create or edit a(n) AHandlingUnitItemDeliveryType,
 * - [AHandlingUnitItemDeliveryDetailFragment]: to display the details of a(n) AHandlingUnitItemDeliveryType,
 * - [AHandlingUnitItemDeliveryListFragment]: to list AHandlingUnitItemDelivery.
 *
 ...
 * what to do.
 */
class AHandlingUnitItemDeliveryActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<AHandlingUnitItemDeliveryType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AHandlingUnitItemDeliveryListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: AHandlingUnitItemDeliveryType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link AHandlingUnitItemDeliveryCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: AHandlingUnitItemDeliveryType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AHandlingUnitItemDeliveryDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, AHandlingUnitItemDeliveryDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = AHandlingUnitItemDeliveryCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: AHandlingUnitItemDeliveryType?) {
        val updateFragment = AHandlingUnitItemDeliveryCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as AHandlingUnitItemDeliveryActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[AHandlingUnitItemDeliveryTypeViewModel::class.java]
