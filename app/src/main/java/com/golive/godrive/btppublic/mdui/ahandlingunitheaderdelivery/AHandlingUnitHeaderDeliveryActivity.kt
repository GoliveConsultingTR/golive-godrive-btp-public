...
import com.golive.godrive.btppublic.databinding.ActivityEntityitemBinding
 ...
 * - [AHandlingUnitHeaderDeliveryCreateFragment]: to create or edit a(n) AHandlingUnitHeaderDeliveryType,
 * - [AHandlingUnitHeaderDeliveryDetailFragment]: to display the details of a(n) AHandlingUnitHeaderDeliveryType,
 * - [AHandlingUnitHeaderDeliveryListFragment]: to list AHandlingUnitHeaderDelivery.
 *
 ...
 * what to do.
 */
class AHandlingUnitHeaderDeliveryActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<AHandlingUnitHeaderDeliveryType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AHandlingUnitHeaderDeliveryListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: AHandlingUnitHeaderDeliveryType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link AHandlingUnitHeaderDeliveryCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: AHandlingUnitHeaderDeliveryType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AHandlingUnitHeaderDeliveryDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, AHandlingUnitHeaderDeliveryDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = AHandlingUnitHeaderDeliveryCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: AHandlingUnitHeaderDeliveryType?) {
        val updateFragment = AHandlingUnitHeaderDeliveryCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as AHandlingUnitHeaderDeliveryActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[AHandlingUnitHeaderDeliveryTypeViewModel::class.java]
