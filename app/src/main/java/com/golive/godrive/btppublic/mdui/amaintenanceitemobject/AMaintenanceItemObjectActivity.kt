 ...
 * - [AMaintenanceItemObjectCreateFragment]: to create or edit a(n) AMaintenanceItemObjectType,
 * - [AMaintenanceItemObjectDetailFragment]: to display the details of a(n) AMaintenanceItemObjectType,
 * - [AMaintenanceItemObjectListFragment]: to list AMaintenanceItemObject.
 *
 ...
 * what to do.
 */
class AMaintenanceItemObjectActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<AMaintenanceItemObjectType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AMaintenanceItemObjectListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: AMaintenanceItemObjectType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link AMaintenanceItemObjectCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: AMaintenanceItemObjectType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AMaintenanceItemObjectDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, AMaintenanceItemObjectDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = AMaintenanceItemObjectCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: AMaintenanceItemObjectType?) {
        val updateFragment = AMaintenanceItemObjectCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as AMaintenanceItemObjectActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[AMaintenanceItemObjectTypeViewModel::class.java]
