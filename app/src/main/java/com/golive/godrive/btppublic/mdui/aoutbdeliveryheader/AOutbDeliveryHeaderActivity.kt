 ...
 * - [AOutbDeliveryHeaderCreateFragment]: to create or edit a(n) AOutbDeliveryHeaderType,
 * - [AOutbDeliveryHeaderDetailFragment]: to display the details of a(n) AOutbDeliveryHeaderType,
 * - [AOutbDeliveryHeaderListFragment]: to list AOutbDeliveryHeader.
 *
 ...
 * what to do.
 */
class AOutbDeliveryHeaderActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<AOutbDeliveryHeaderType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryHeaderListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: AOutbDeliveryHeaderType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link AOutbDeliveryHeaderCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: AOutbDeliveryHeaderType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, AOutbDeliveryHeaderDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, AOutbDeliveryHeaderDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = AOutbDeliveryHeaderCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: AOutbDeliveryHeaderType?) {
        val updateFragment = AOutbDeliveryHeaderCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as AOutbDeliveryHeaderActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[AOutbDeliveryHeaderTypeViewModel::class.java]
